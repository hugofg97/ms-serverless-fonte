...
const {  ISessionFindByName, ISession, ISessionFindAll, ISessionPagination, ISessionCountPerTag } = require("./ISession");
const { IVideoPagination, IVideoCountPerSession } = require("../videos/IVideo");

module.exports = class  ISessionService {
  async create(session) {
    const existsSession = await new ISessionFindByName(session).find(serviceLocator);
    if(existsSession) throw { error: 409, field: `Sessão : ${existsSession?.name}` };
    return await new ISession(session).create(serviceLocator);
  }

  async findByName({ name }) {
    const sessionExists = await new ISessionFindByName({name}).find(serviceLocator);
  ...
  async findAll(
    ...
    const allSessions = await new ISessionFindAll( { limit,tag }).find(serviceLocator);
  ...
  async pagination(
    {tag, page, subscriberId,isSubscriber }
  ) {
  ...
  async #videosPerSession({
    ...
    for await (const [index, value] of Object.entries(allSessions)) {
      allSessions[index].videosCount = await new IVideoCountPerSession({sessionId: allSessions[index]._id}).count(serviceLocator);
